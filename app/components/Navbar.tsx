 ...
 * 
 * Responsive tiers:
 *   Watch  (< 220px)  — logo-only micro bar, no hamburger
 *   Mobile (< 1024px) — logo + hamburger, full overlay menu
 *   Desktop (≥ 1024px)— full desktop nav with dropdowns
 */
...
const Navbar = () => {
  ...
  useEffect(() => {
    const checkWatch = () => setIsWatch(window.innerWidth < 220);
    checkWatch();
    ...
    return () => {
      window.removeEventListener('resize', checkWatch);
  ...
  // ── Apple Watch: micro logo-only bar ─────────────────────────────────────
  // At < 220px there's no room for a hamburger menu or any nav links.
  // We render just the logo mark so the brand is still identifiable.
  // The "AL ZAHRA HUMAN RESOURCES" text that was bleeding was the logo
  // subtitle — on watch we suppress the text entirely and show only the icon.
  if (isWatch) {
    return (
      <header
        className="fixed top-0 left-0 w-full z-[100] flex items-center justify-between bg-black/90 backdrop-blur-md"
        style={{ height: "28px", padding: "0 6px" }}
        id="site-navbar"
      >
        <Link href="/" className="flex items-center gap-1">
          <div className="brightness-0 invert relative" style={{ width: 14, height: 14 }}>
            <Image src="/logo.png" alt="Al Zahra" fill className="object-contain" />
          </div>
          {/* No text on watch — icon only */}
        </Link>
        {/* No hamburger on watch — no room and it's not useful */}
      </header>
    );
  }

  // ── Standard navbar (mobile + tablet + desktop) ───────────────────────────
  return (
    <>
      <header
        id="site-navbar"
        className={`fixed top-0 left-0 w-full z-[100] transition-all duration-300 font-sans text-white ${
          isScrolled
            ? 'bg-black/95 backdrop-blur-md shadow-lg py-2'
            : 'bg-transparent py-4 [@media(max-height:450px)]:py-1'
        }`}
      >
        {/* Desktop utility bar — only shown when not scrolled */}
        <div
          className={`hidden lg:flex justify-center items-center w-full pb-2 mb-2 border-b border-white/10 text-xs font-[family-name:var(--font-open-sans)] transition-all duration-300 ${
        ...
        >
          <div className="flex items-center gap-2 opacity-90 font-semibold tracking-wide">
            ...
            <a href={CONTACT_INFO.phoneLink} className="hover:text-[#39B54A] transition-colors flex items-center gap-1">
              📞 {CONTACT_INFO.phone}
            </a>
            <span className="text-white/30 px-2">|</span>
            <a href={CONTACT_INFO.emailLink} className="hover:text-[#39B54A] transition-colors flex items-center gap-1">
        ...
        <div className="w-full px-4 lg:px-6 xl:px-12 flex justify-between items-center relative z-20">
              ...
              <div className="absolute top-full -left-[100px] xl:left-0 w-[400px] bg-white text-[#1B2B21] p-4 rounded-b-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all translate-y-2 group-hover:translate-y-0 border-t-2 border-[#39B54A] flex gap-4">
                <div className="flex-1 text-left">
                  ...
                  {COUNTRIES.middleEast.map(c => (
                    <Link key={c.href} href={c.href} className="block px-2 py-2 hover:bg-[#E8F5E9] rounded text-xs font-semibold">{c.name}</Link>
                  ))}
                </div>
          ...
          {/* Hamburger — hidden on watch (handled above), shown on mobile/tablet */}
          <button
            className="lg:hidden p-2 z-50 flex flex-col gap-1.5"
      ...
      {/* MOBILE OVERLAY */}
      <div
        className={`fixed inset-0 bg-black/98 backdrop-blur-3xl lg:hidden z-[110] flex flex-col transition-all duration-300 ${
      ...
      >
        <div className="w-full flex justify-between items-center px-4 py-4 border-b border-white/10 bg-[#006837]/20 shrink-0">
          <Link href="/" className="flex items-center gap-2" onClick={closeMenu}>
            <div className="brightness-0 invert relative w-6 h-6">
          ...
          <div className="flex items-center gap-2">
            <Link href="/contact" onClick={closeMenu} className="px-4 py-2 bg-[#39B54A] text-white rounded-full font-bold uppercase text-[10px]">
              Hire Talent
            </Link>
            <button className="relative w-8 h-8" onClick={closeMenu} aria-label="Close menu">
          ...
          <div className="flex flex-col gap-1 max-w-lg mx-auto pb-10">
            ...
            <MobileAccordion label="Industries" href="/industries" isOpen={openAccordion === 'ind'} onToggle={() => toggleAccordion('ind')} onLinkClick={closeMenu}>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pl-4 border-l-2 border-[#39B54A] ml-2">
                {INDUSTRIES.map(item => (
                  <Link key={item.href} href={item.href} onClick={closeMenu} className="text-sm font-semibold text-white/80 py-1">{item.name}</Link>
                ))}
              </div>
            </MobileAccordion>
              ...
              <div className="flex flex-col gap-4 pl-4 border-l-2 border-[#39B54A] ml-2">
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {COUNTRIES.middleEast.map(c => (
                      <Link key={c.href} href={c.href} onClick={closeMenu} className="text-sm font-semibold text-white/70">{c.short}</Link>
                    ))}
                  </div>
                </div>
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {COUNTRIES.global.map(c => (
                      <Link key={c.href} href={c.href} onClick={closeMenu} className="text-sm font-semibold text-white/70">{c.name}</Link>
                    ))}
                  </div>
                </div>
              </div>
            ...
            <div className="mt-8">
              <Link href="/partner" onClick={closeMenu} className="block w-full py-4 border border-white/30 text-center rounded-lg font-bold text-sm uppercase tracking-widest text-white/80">
                Partner With Us
              </Link>
            </div>
          </div>
...
const MobileAccordion = ({
  ...
  label: string; href: string; isOpen: boolean; onToggle: () => void; onLinkClick: () => void; children: React.ReactNode;
}) => (
  <div className="w-full border-b border-white/5 py-1">
    <div className="w-full flex justify-between items-center py-2">
      ...
      <button onClick={onToggle} className="p-2 ml-4" aria-label={`Toggle ${label}`}>
